    ...
    try:
        ...
        for tool_name, expected_server in test_tools:
            ...
            try:
                ...
                result = universal_tool(tool_name=tool_name, query="test" if tool_name == 'search_nodes' else ".")
                
                if result and not (isinstance(result, dict) and result.get('success') == False):
                    print(f"   ✅ {tool_name}: Mapped correctly to {expected_server}")
                    mapping_results[tool_name] = True
                else:
                    print(f"   ⚠️ {tool_name}: Server found but tool execution failed (expected for testing)")
                    mapping_results[tool_name] = True  # Mapping works even if execution fails
                ...
                if "not found" in error_msg:
                    ...
                    print(f"   ⚠️ {tool_name}: Execution error (mapping may be correct) - {error_msg}")
                    mapping_results[tool_name] = True  # Tool was found, execution error is different issue
        
        print(f"📊 Mapping Test Results: {sum(mapping_results.values())}/{len(mapping_results)} tools mapped correctly")
...
async def test_agent_mode_simulation():
    ...
    try:
        ...
        try:
            ...
            execution_time = time.time() - start_time
            print(f"❌ Agent execution failed after {execution_time:.2f} seconds: {agent_error}")
            
            # Check if the error is the old "Server 'universal' not found" error
            if "Server 'universal' not found" in str(agent_error):
                print("💥 CRITICAL: Still getting 'universal' server error - fix not working!")
                return False
    ...
    try:
        ...
        for tool_info in failing_tools:
            ...
            try:
                ...
                if result and not (isinstance(result, dict) and "Server 'universal' not found" in str(result)):
                ...
                if "Server 'universal' not found" in error_msg:
                    print(f"   💥 {tool_name}: CRITICAL - Still trying to use 'universal' server!")
                    tool_results[tool_name] = False
                else:
        ...
        print(f"📊 Individual Tool Test Results: {success_count}/{total_count} tools working")
...
async def run_comprehensive_test():
    ...
    mapping_success = len([r for r in mapping_results.values() if r]) >= len(mapping_results) * 0.8
    
    # Test 3: Individual MCP Tools
    tool_results = await test_individual_mcp_tools()
    tools_success = len([r for r in tool_results.values() if r]) >= len(tool_results) * 0.8
