...
class APIErrorTestSuite:
    ...
    def test_502_error_handling(self) -> bool:
        ...
        try:
            ...
            mock_error = openai.APIStatusError("Error code: 502", response=mock_response, body=mock_body)
            
            with patch.object(integration.client.chat.completions, 'create', side_effect=mock_error):
                result = integration.generate_text("Test message")
                
                # Should return fallback response, not crash
                if result and len(result) > 10:
                    self.log_test("502 Error Handling", "PASS", "Returned fallback response")
                    return True
                else:
        ...
        try:
            ...
            def mock_502_then_success(*args, **kwargs):
                ...
                if attempt_count <= 2:  # Fail first 2 attempts
                    ...
                    mock_response.status_code = 502
                    mock_response.headers = {}
                    raise openai.APIStatusError("Error code: 502", response=mock_response, body="Bad Gateway")
                else:
            ...
            with patch.object(integration.client.chat.completions, 'create', side_effect=mock_502_then_success):
                ...
                if result == "Success after retry" and elapsed_time > 1:
                    self.log_test("Retry Logic", "PASS", f"Succeeded after {attempt_count} attempts in {elapsed_time:.1f}s")
                    return True
                else:
    ...
    def test_circuit_breaker(self) -> bool:
        ...
        try:
            ...
            mock_response.status_code = 502
            mock_response.headers = {}
            mock_error = openai.APIStatusError("Error code: 502", response=mock_response, body="Bad Gateway")
            ...
            with patch.object(integration.client.chat.completions, 'create', side_effect=mock_error):
                ...
                if OpenAIIntegration._circuit_open:
                    ...
                    self.log_test("Circuit Breaker", "FAIL", "Circuit breaker did not open")
                    return False
    ...
    def test_fallback_responses(self) -> bool:
        ...
        try:
            ...
            if "message_type" not in classification_fallback:
                self.log_test("Fallback Responses", "FAIL", "Classification fallback invalid")
                return False
            ...
            if len(general_fallback) < 10:
                self.log_test("Fallback Responses", "FAIL", "General fallback too short")
                return False
    ...
    def test_recovery(self) -> bool:
        ...
        try:
            ...
            with patch.object(integration.client.chat.completions, 'create', return_value=mock_response):
