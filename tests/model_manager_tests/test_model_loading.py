...
class TestModelManagerLoading(unittest.TestCase):
    ...
    def test_load_valid_model(self, mock_popen):
        ...
        mock_popen.assert_called_once_with("ollama ps", shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
    ...
    def test_model_switching(self, mock_os_system, mock_popen):
        ...
        mock_process = MagicMock()
        mock_process.stdout.read.return_value.decode.return_value = f"{config.DEFAULT_MODEL}"
        mock_popen.return_value = mock_process
    ...
    def test_same_model_already_loaded(self, mock_popen):
        ...
        with patch('builtins.print') as mock_print:
            ModelManager.load_model(config.DEFAULT_MODEL)
            mock_print.assert_any_call(f"\t[log]Model {config.DEFAULT_MODEL} is already loaded.")
    ...
    def test_model_list_validation(self, mock_popen):
        ...
        expected_models = [config.DEFAULT_MODEL, config.CYPHER_MODEL, config.CLASSIFIER_MODEL]
