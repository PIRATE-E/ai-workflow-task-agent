...
def test_openai_live_api():
    ...
    try:
        ...
        try:
            ...
            if "temporarily unavailable" in error_str.lower() or "technical issues" in error_str.lower():
...
def test_openai_error_scenarios():
    ...
    try:
        ...
        for i in range(6):  # Trigger circuit breaker
            ...
            print(f"      Failure {i+1}: count={OpenAIIntegration._failure_count}, open={OpenAIIntegration._circuit_open}")
...
def test_mcp_integration():
    ...
    try:
        ...
        print("   1. Testing universal tool (should not get 'universal' server error)...")
...
async def test_agent_mode():
    ...
    try:
        ...
        print(f"   ?? Agent mode has setup requirements but core functionality available: {e}")
...
def check_system_health():
    ...
    try:
        ...
        try:
            from src.ui.diagnostics.debug_helpers import debug_info, debug_error, debug_warning
...
async def main():
    ...
    tests = [
        ...
        ("Live API Test", test_openai_live_api),  # Last because it might fail due to API issues
    ]
