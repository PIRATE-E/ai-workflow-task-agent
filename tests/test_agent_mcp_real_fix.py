...
def setup_mcp_servers():
    ...
    try:
        ...
        servers_config = [
            ("github", Command.NPX, ["-y", "@modelcontextprotocol/server-github@latest"]),
            ("git", Command.UVX, ["mcp-server-git"]),
            ("filesystem", Command.NPX, ["-y", "@modelcontextprotocol/server-filesystem@latest", "C:\\Users\\pirat\\PycharmProjects\\AI_llm"]),
            ("memory", Command.NPX, ["-y", "@modelcontextprotocol/server-memory@latest"]),
            ("puppeteer", Command.NPX, ["-y", "@modelcontextprotocol/server-puppeteer@latest"]),
            ("sentry", Command.NPX, ["-y", "mcp-remote@latest", "https://mcp.sentry.dev/mcp"]),
        ...
        for tool_test in failing_tools:
            ...
            try:
                ...
                if isinstance(result, dict) and "Server 'universal' not found" in str(result.get('error', '')):
                    print(f"   ? {tool_name}: CONFIRMED - 'universal' server error reproduced")
                ...
                if "Server 'universal' not found" in error_str:
                    print(f"   ? {tool_name}: CONFIRMED - 'universal' server error reproduced via exception")
                    return True  # We successfully reproduced the error
                else:
...
def apply_fix():
    ...
    try:
        ...
        universal_file_path = "src/tools/lggraph_tools/tools/mcp_integrated_tools/universal.py"
        ...
        print("   Current code calls: MCP_Manager.call_mcp_server('universal', tool_name, arguments)")
            ...
            for test_case in test_cases:
                ...
                try:
                    ...
                    if isinstance(result, dict):
                        if result.get('success') == False and "Server 'universal' not found" in str(result.get('error', '')):
                            print(f"   ? {tool_name}: Still trying to use 'universal' server - FIX NOT WORKING")
                            return False
                        elif result.get('success') == False and "not running" in str(result.get('error', '')):
                            print(f"   ? {tool_name}: Fix working - now correctly maps to proper server")
                            return True
                        elif result.get('success') == True:
                            print(f"   ? {tool_name}: Fix working - tool executed successfully")
                            return True
                        else:
                    ...
                    if "Server 'universal' not found" in error_str:
                        ...
                        print(f"   ? {tool_name}: Fix working - different error indicates server mapping worked")
                        return True
        else:
...
def simulate_agent_mode():
    ...
    try:
        ...
        try:
            ...
            if isinstance(read_result, dict) and "Server 'universal' not found" in str(read_result.get('error', '')):
                print("   ? FAILURE: read_graph still tries to use 'universal' server")
                print("   ?? This is the exact error from the error log!")
                return False
            else:
                print("   ? SUCCESS: read_graph now works (no 'universal' server error)")
            ...
            if "Server 'universal' not found" in str(e):
                ...
                print("   ? SUCCESS: read_graph now works (different error indicates fix working)")
        
        # Step 2: Agent would call search_nodes (this also fails in error log)
        print("   Step 2: Agent calls search_nodes...")
        try:
            search_result = universal_tool(tool_name="search_nodes", query="ai work flow one")
            
            if isinstance(search_result, dict) and "Server 'universal' not found" in str(search_result.get('error', '')):
                print("   ? FAILURE: search_nodes still tries to use 'universal' server")
                print("   ?? This is the exact error from the error log!")
                return False
            else:
                print("   ? SUCCESS: search_nodes now works (no 'universal' server error)")
            ...
            if "Server 'universal' not found" in str(e):
                ...
                print("   ? SUCCESS: search_nodes now works (different error indicates fix working)")
