...
def event_emitter(event_type, variable_name=None):
    ...
    def decorator(func):
        ...
        def wrapper(self, *args, **kwargs):
            ...
            old_value = getattr(self, var_name, None) if hasattr(self, var_name) else None
            
            # Execute the original method
            result = func(self, *args, **kwargs)
            
            # Get new value after method execution
            new_value = getattr(self, var_name, None) if hasattr(self, var_name) else (args[0] if args else None)
...
class EventAwareBase:
    ...
    def __setattr__(self, name, value):
        ...
        if (getattr(self, '_initializing', True) or 
...
def selective_event_aware(*attribute_names):
    ...
    def decorator(cls):
        ...
        def selective_setattr(self, name, value):
            ...
            if (name in attribute_names and 
                old_value != value and 
...
def is_event_aware(obj_or_class):
    ...
    return (hasattr(cls, '_is_event_aware') or 
            issubclass(cls, EventAwareBase) or
            hasattr(cls, '_watched_attributes'))
