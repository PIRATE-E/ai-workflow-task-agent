...
class SocketManager:
    ...
    def get_socket_con():
        ...
        return (
            ...
            if ContextRegistry.get().get_settings().enable_socket_logging
            else None
        )
        ...
        if self._socket_connection is None or not self._socket_connection.is_connected():
            ...
            if self.start_log_server():
                ...
                try:
                    ...
                    client_socket.connect((ContextRegistry.get().get_settings().socket_host, ContextRegistry.get().get_settings().socket_port))
