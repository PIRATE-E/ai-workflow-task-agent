...
def update_config_file(new_mode):
    ...
    try:
        ...
        if 'LOG_DISPLAY_MODE' in content:
            ...
            replacement = f'LOG_DISPLAY_MODE = os.getenv("LOG_DISPLAY_MODE", "{new_mode}")'
...
def show_troubleshooting():
    ...
    table.add_row(
        ...
        "Set ENABLE_SOCKET_LOGGING=True in config.py"
    )
    table.add_row(
        "Port already in use",
        "Change SOCKET_PORT in config.py or stop other processes"
    )
    table.add_row(
